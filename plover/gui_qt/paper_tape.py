...
class PaperTape(QDialog, Ui_PaperTape, WindowState):
    ...
    def _setup_system(self, config):
        ...
        self._all_keys = ''.join(x[0].strip('-') for x in
                                 sorted(system.KEY_ORDER.items(),
                                        key=lambda x: x[1]))
        self._reverse_numbers = {v: k for k, v in system.NUMBERS.items()}
    ...
    def _paper_format(self, stroke):
        ...
        if any(key in self._reverse_numbers for key in keys):
            keys.append('#')
        for key in keys:
            if key in self._reverse_numbers:
                key = self._reverse_numbers[key]
